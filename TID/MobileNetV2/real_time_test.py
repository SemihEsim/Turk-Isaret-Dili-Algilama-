"""
3_realtime_test.py — MobileNetV2 Gerçek Zamanlı Kamera Testi
=============================================================
Çalıştır: python 3_realtime_test.py
...
def draw_header(frame, fps, roi_ratio):
    ...
    cv2.putText(frame, "MobileNetV2 — TID El Alfabesi",
                (w//2-210, 38), cv2.FONT_HERSHEY_DUPLEX, 1.0, (255,255,255), 2)
...
def draw_footer(frame, history):
    ...
    cv2.putText(frame, "Q:Cik  S:Ekran goruntüsü  R:Temizle  +/-:ROI boyutu",
...
def run():
    ...
    print(f"✅ Model hazır! ({len(class_names)} sınıf)\n")
    ...
    while True:
        ...
        cv2.imshow("TID — MobileNetV2", frame)

        key = cv2.waitKey(1) & 0xFF
        if key == ord('q'):
            break
        elif key == ord('s'):
            fn = f"screenshot_{shot_n}.png"
